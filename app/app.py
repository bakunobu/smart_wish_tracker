...
def index():
    ...
    return render_template("index.html", entries=entries, tasks=tasks)
...
def create_task():
    ...
    try:
        ...
        with get_db() as conn:
            ...
            # Insert new task
            cursor = conn.execute("INSERT INTO tasks (name, position) VALUES (?, ?)", (name, new_position))
            task_id = cursor.lastrowid
            conn.commit()

        return jsonify({"success": True, "data": {"id": task_id, "name": name, "position": new_position}}), 201
...
def update_task(task_id):
    ...
    try:
        ...
        return jsonify({"success": True, "data": {"id": task_id, "name": name, "position": task["position"]}})
...
def edit_quest(quest_id):
    try:
        with get_db() as conn:
            ...
            return render_template("edit_quest.html", quest=dict(quest))
    except Exception as e:
...
def delete_task(task_id):
    ...
    try:
        with get_db() as conn:
            ...
            conn.execute("UPDATE tasks SET position = position - 1 WHERE position > ?", (deleted_position,))
            conn.commit()
...
def get_suggestions():
    ...
    try:
        ...
        with get_db() as conn:
            cursor = conn.execute("SELECT name FROM tasks WHERE name LIKE ? LIMIT 5", (f"%{query}%",))
...
def get_quests():
    ...
    try:
        with get_db() as conn:
            cursor = conn.execute(
                "SELECT id, short_name, description, quest_type, stack_duration_days, stack_min_time_per_day, duration_hours, complete_result, reward_url, reward_image, position, is_active FROM quests ORDER BY position"
            )
...
def delete_quest(quest_id):
    ...
    try:
        with get_db() as conn:
            ...
            conn.execute(
                "UPDATE quests SET position = position - 1 WHERE position > (SELECT position FROM quests WHERE id = ?)",
                (quest_id,),
            )
            conn.commit()
...
def create_quest():
    ...
    try:
        ...
        return jsonify({"success": True, "data": {"id": quest_id, "position": new_position}}), 201
...
def start_timer_session():
    ...
    try:
        ...
        return jsonify({"success": True, "session_id": session_db_id, "message": "Timer session started"})
    except Exception as e:
        return jsonify({"success": False, "error": str(e)}), 500
...
def stop_timer_session():
    ...
    try:
        ...
        return jsonify({"success": True, "message": "Timer session stopped"})
    except Exception as e:
        return jsonify({"success": False, "error": str(e)}), 500
...
def complete_timer_session():
    ...
    try:
        ...
        return jsonify({"success": True, "message": "Timer session completed successfully"})
    except Exception as e:
        return jsonify({"success": False, "error": str(e)}), 500
    ...
    try:
        ...
        with get_db() as conn:
            if task_name:
                ...
                return jsonify({"success": True, "stats": all_stats, "count": len(all_stats)})
...
def export_timer_data():
    ...
    try:
        ...
        export_data = {"export_timestamp": datetime.now().isoformat(), "sessions": sessions, "statistics": statistics}

        if format_type == "json":
            response = jsonify(export_data)
            response.headers[
                "Content-Disposition"
            ] = f'attachment; filename=timer_history_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json'
            return response
        else:
